package com.miqiang.lixian.util.query;

import cn.hutool.core.util.StrUtil;
import com.alibaba.fastjson.JSON;
import com.baomidou.mybatisplus.annotation.TableField;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.miqiang.lixian.util.*;
...
public class QueryGenerator {
    ...
    public static <T> QueryWrapper<T> initQueryWrapper(T searchObj,Map<String, String[]> parameterMap,String ...prefix){
        long start = System.currentTimeMillis();
        QueryWrapper<T> queryWrapper = new QueryWrapper<T>();
        installMplus(queryWrapper, searchObj, parameterMap,prefix);
        log.debug("---查询条件构造器初始化完成,耗时:"+(System.currentTimeMillis()-start)+"毫秒----");
        return queryWrapper;
    }
     ...
     */
    public static void installMplus(QueryWrapper<?> queryWrapper,Object searchObj,Map<String, String[]> parameterMap,String ...prefix) {
		...
		*/
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            try {
                ...
                if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
                    ...
                    if(prefix.length > 0 && StrUtil.isNotBlank(prefix[0])){
                        addEasyQueryPrefix(queryWrapper, column, rule, value,prefix[0]);
                    }else {
        ...
        // 排序逻辑 处理
        doMultiFieldsOrder(queryWrapper, parameterMap,prefix);
    ...
    //多字段排序 TODO 需要修改前端
    public static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap,String ...prefix) {
        ...
        if (StringUtils.isNotBlank(column) && StringUtils.isNotBlank(order)) {
            ...
            if (order.toUpperCase().indexOf(ORDER_TYPE_ASC)>=0) {
                queryWrapper.orderByAsc(((prefix.length > 0) ? prefix[0] + "." : "") + ConvertUtils.camelToUnderline(column));
            } else {
                queryWrapper.orderByDesc(((prefix.length > 0) ? prefix[0] + "." : "") + ConvertUtils.camelToUnderline(column));
            }
        }
    }
            ...
            try {
                ...
                queryWrapper.and(andWrapper -> {
                    for (int i = 0; i < conditions.size(); i++) {
                        ...
                        if (StringUtils.isNotBlank(rule.getField())
                                && StringUtils.isNotBlank(rule.getRule())
                                && StringUtils.isNotBlank(rule.getVal())) {
    ...
    private static void addEasyQueryPrefix(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value,String preFix) {
        if (value == null || rule == null || ConvertUtils.isEmpty(value)) {
            return;
        }
        name = ConvertUtils.camelToUnderline(name);
